...
| Tables and cell text | Exact canonical `[tbl]`/`[h]`/`[w]`/`[data]` shapes type dimensions, width/gutter weights, header rows, alignment, and independently validated horizontal merges; border/shading/protection/tails remain typed but semantically partial | Proportional-width reflow with header rows, alignment, and validated spans; unsupported styling remains diagnostic | Simple tables/TSV |
...
| Page-layout headers/footers | Typed odd/even IR with bounded `[lyfrm]` geometry; nested and sibling stream shapes supported | PDF/ODT/DOCX promote a narrow unambiguous, layout-matched, size-bounded subset to repeated page furniture; other known variants reflow without body labels, nonempty unknown/invalid variants remain visible, and empty furniture stays in diagnostics/JSON | Known contents reflow without labels; audit labels are opt-in |
| Anchored body frames | Typed `Frame` at the original zero-based `<:tN>`/`<:AN>` anchor location; opaque header tails and `[frmlay]` fields are classified | Contents rendered once in source order without a generic body label; bounded width may guide safe reflow, but absolute coordinates and overlap are not reproduced | Source-order contents; audit label is opt-in |
| Fixed-page and repeating frames | Page/flag/rectangle metadata typed when bounded; raw fields retained | Known contents reflow without a generic body label; original fixed/repeating placement is not reproduced; unknown/invalid structure stays labelled | Source-order contents; audit label is opt-in |
